...
typedef struct {
    ...
    uint32_t caravan_state; /* +0x12c : packed caravan progress (see CAVAN_* macros) */
} Raid;

/* =========================================================
 * Caravan system
 * caravan_state uint32 packing:
 *   bits  [1:0]  = phase (CAVAN_IDLE/INFLIGHT/SUCCESS/FAIL)
 *   bits  [5:2]  = workers - 1  (1-8 stored as 0-7)
 *   bits  [8:6]  = guards        (0-6)
 *   bits [15:9]  = timer         (0-60 ticks)
 *   bits [20:16] = food/100 - 1  (100-2000 stored as 0-19)
 *   bit  [21]    = blueprint found flag
 *   bits [23:22] = blueprint wonder index (0-2)
 *   bits [31:24] = spare
 * ========================================================= */
#define CAVAN_IDLE     0
#define CAVAN_INFLIGHT 1
#define CAVAN_SUCCESS  2
#define CAVAN_FAIL     3

#define CAVAN_PHASE(cs)     ((cs) & 3)
#define CAVAN_WORKERS(cs)   ((((cs) >> 2) & 0xF) + 1)
#define CAVAN_GUARDS(cs)    (((cs) >> 6) & 7)
#define CAVAN_TIMER(cs)     (((cs) >> 9) & 0x7F)
#define CAVAN_FOOD_H(cs)    ((((cs) >> 16) & 0x1F) + 1)
#define CAVAN_BP_FOUND(cs)  (((cs) >> 21) & 1)
#define CAVAN_BP_IDX(cs)    (((cs) >> 22) & 3)

#define CARAVAN_TRIP_TICKS   60
#define CARAVAN_MAX_WORKERS  8
#define CARAVAN_MAX_GUARDS   6
#define CARAVAN_FOOD_STEP    100
#define CARAVAN_FOOD_MAX     2000
...
#define WONDER_BP_BIT_BASE      43
#define WONDER_NONE_VAL         0xFF

/* Unpack prestige._pad which stores active wonder construction state:
 *   bits  [7:0]  = wonder index (0-2) or WONDER_NONE_VAL (0xFF) if idle
...
#define WONDER_HAS_BP(t2, idx)  (((t2) >> (43 + (idx))) & 1ULL)
